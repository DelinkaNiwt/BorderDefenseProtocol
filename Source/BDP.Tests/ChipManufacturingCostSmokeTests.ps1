...
Assert-True ($calculatorText -match 'additionalCost') "总材料必须最多累加一次枪壳材料。"
Assert-True ($calculatorText -match 'additionalWorkAmount') "工作量必须累加动作与枪壳附加工时。"
...
Assert-True ($universeText -match 'DefDatabase<ChipGunShellDef>') "材料槽全集必须包含所有枪壳出现过的材料。"
...
# 加载正式程序集，实际构造基础配方、两个动作与一个枪壳验证累加结果。
...
$shellType = $contentAssembly.GetType("BDP.Content.Assembly.ChipManufacturing.Defs.ChipGunShellDef", $true)
...
Assert-Equal ($cost.CountOf($steel)) 35 "动作与枪壳的同类钢铁必须相加。"
Assert-Equal ($cost.CountOf($silver)) 40 "两个动作的同类白银必须相加。"
Assert-Equal $cost.WorkAmount 2600 "工作量必须按基础、动作和枪壳相加。"
Assert-Equal $cost.Ingredients.Count 3 "同类材料必须合并为一个条目。"

Write-Host "PASS: 芯片单枚成本由基础配方、全部动作和可选枪壳简单累加，同类材料合并。"
