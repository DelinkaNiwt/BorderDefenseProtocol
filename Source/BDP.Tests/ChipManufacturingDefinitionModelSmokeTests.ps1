...
$gunShellsPath = Join-Path $modRoot "1.6\Content\Defs\ChipGunShellDef\Presets.xml"

$classificationText = Get-Utf8Text $classificationPath
$actionText = Get-Utf8Text $actionPresetsPath
$gunShellText = Get-Utf8Text $gunShellsPath
...
$requiredFiles = @("ChipProfessionDef.cs", "ChipActionPresetDef.cs", "ChipGunShellDef.cs", "ChipGunShellOverrides.cs")
...
Assert-True ($definitionText -match 'List<ThingDefCountClass>\s+additionalCost') "枪壳缺少附加材料字段。"
Assert-True ($definitionText -match 'List<ChipProfessionDef>\s+compatibleProfessions') "枪壳缺少职业兼容字段。"
...
Assert-True ($gunShellText -match 'BDP\.Content\.Assembly\.ChipManufacturing\.Defs\.ChipGunShellDef') "枪壳 XML 必须使用 Content 定义类型。"
Assert-True ($gunShellText -notmatch 'BDP\.Core\.Chips\.GunClassDef|compatibleTags|additionalWorkTicks') "枪壳 XML 不得继续使用 Core 旧定义字段。"

Write-Host "PASS: Content 定义了五主分类、唯一职业、动作材料/工作量和枪壳兼容模型。"
