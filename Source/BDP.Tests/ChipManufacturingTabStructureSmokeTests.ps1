...
Assert-True ($windowText -match 'forms\.Count\s*\*\s*PresetRowHeight') "武装型区域必须按实际条目数计算高度。"
Assert-True ($windowText -match 'MinimumActionSectionHeight') "左栏必须为动作列表保留最小可视高度。"
Assert-True ($windowText -match 'Mathf\.Min\s*\(\s*desiredArmamentFormHeight') "武装型区域必须受左栏实际可用高度限制。"
Assert-True ($windowText -match 'DrawArmamentFormSection[\s\S]*DrawActionSection') "武装型区必须位于动作列表上方。"
