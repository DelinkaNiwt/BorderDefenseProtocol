...
Assert-True ($modelText -match 'ArmamentFormAdjustments') "武装型修正必须单独显示一次。"
Assert-True ($modelText -match 'ArmamentFormMetrics') "武装型共性属性必须单独保存为指标组。"
...
Assert-True ($builderText -match 'AddArmamentFormMetrics') "武装型共性属性必须由预览构建器统一生成。"
Assert-True ($builderText -match 'BurstShotCount') "武装型特征必须包含子弹数量。"
Assert-True ($builderText -notmatch 'AddMultiplier\s*\(\s*model\s*,\s*"ProjectileDamage"') "武装型特征不得重复显示投射物伤害倍率。"
Assert-True ($builderText -match 'showArmamentFormCommonMetrics\s*=\s*armamentForm\s*==\s*null') "动作属性构建必须区分已选武装型场景。"
...
Assert-True ($panelText -match 'DrawHeader[\s\S]*GameFont\.Small') "规格、武装型和动作段标题必须使用小号层级。"
...
Assert-True ($panelText -match 'BDP_ChipManufacturing_Preview_ArmamentFormAdjustments"\.Translate\s*\(') "武装型修正标题必须翻译后再绘制。"
Assert-True ($panelText -match 'ArmamentFormMetrics[\s\S]*DrawMetric') "武装型共性属性必须以条形图指标形式绘制。"
Assert-True ($panelText -match 'DrawAdjustmentGrid') "武装型修正必须使用紧凑双列排版。"
Assert-True ($languageText -match '<BDP_ChipManufacturing_ProductLabel>触发器芯片:\{0\}</BDP_ChipManufacturing_ProductLabel>') "无武装型成品名必须使用无空格英文冒号。"
Assert-True ($languageText -match '<BDP_ChipManufacturing_ProductLabelWithArmamentForm>触发器芯片:\{0\}\[\{1\}型\]</BDP_ChipManufacturing_ProductLabelWithArmamentForm>') "带武装型成品名必须使用无空格英文冒号。"
...
Write-Host "PASS: 中栏按同级分组显示芯片规格、一次武装型修正和上下动作形态，数值位于条形图右侧。"
