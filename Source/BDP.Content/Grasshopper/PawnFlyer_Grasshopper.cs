...
namespace BDP.Content.Grasshopper
{
    ...
    {
        /// <summary>多段跳跃链式回调：降落时触发下一段跳跃。</summary>
        public System.Action onLanded;

        /// <summary>子类可覆写为 false 关闭蚱蜢专属起跳/气浪特效。</summary>
        protected virtual bool ShowGrasshopperTrailFx => true;
        ...
        private SimpleCurve GetProgressCurve()
        {
            if (!curveResolved)
            {
                ...
                {
                    // 列出所有字段名，帮助诊断
                    var fields = typeof(PawnFlyerProperties).GetFields(
                        BindingFlags.NonPublic | BindingFlags.Instance);
                    Log.Warning($"[Grasshopper] Field 'progressCurve' not found! All fields: " +
                        string.Join(", ", System.Array.ConvertAll(fields, f => f.Name)));
                }
            }
        ...
        /// </summary>
        protected Vector3 GetLinearPosition()
        {
        ...
        protected override void Tick()
        {
            ...
            if (!launchEffectSpawned && ticksFlying >= 1)
            {
                ...
                // 生成主踏板（稍微放大）
                FleckDef platformDef = DefDatabase<FleckDef>.GetNamed("BDP_GrasshopperPlatform");
                if (platformDef != null)
                    FleckMaker.Static(footPos, Map, platformDef, 0.7f);

                // 生成2个扩散波纹，错开时间产生连续脉冲效果
                FleckDef pulseDef = DefDatabase<FleckDef>.GetNamed("BDP_GrasshopperPulse");
                if (pulseDef != null)
                {
                    for (int i = 0; i < 2; i++)
                    {
            ...
            FleckDef airPuffDef = DefDatabase<FleckDef>.GetNamed("BDP_GrasshopperAirPuff");
            if (airPuffDef != null)
            {
                ...
                {
                    trailSpawned_0 = true;
                    FleckMaker.Static(trailPos, Map, airPuffDef, 1.5f);
                }
        ...
        /// <summary>
        /// 降落时触发：Pawn 已到达目标位置后，检查是否有链式跳跃回调。
        /// 多段跳跃通过此机制实现：每段落地后自动发起下一段。
        /// </summary>
        protected override void RespawnPawn()
        {
            base.RespawnPawn();

            System.Action callback = onLanded;
            onLanded = null;
            if (callback != null)
            {
                callback.Invoke();
