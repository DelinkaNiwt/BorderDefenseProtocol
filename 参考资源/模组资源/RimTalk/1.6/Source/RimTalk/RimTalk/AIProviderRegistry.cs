...
{
	public static readonly Dictionary<AIProvider, ProviderDef> Defs = new Dictionary<AIProvider, ProviderDef>
	{
		{
			AIProvider.Google,
			new ProviderDef
	...
	{
		ProviderDef def;
		return Defs.TryGetValue(p, out def) ? def.EndpointUrl : null;
	}

	public static string GetListModelsUrl(this AIProvider p)
	{
		ProviderDef def;
		return Defs.TryGetValue(p, out def) ? def.ListModelsUrl : null;
	}
}
