...
public class SkillLevelUpPatch
{
	...
	public static void CatchLevelUp(SkillRecord __instance, Pawn ___pawn)
	{
		...
		{
			string prompt = $"{___pawn.Name} leveled up {__instance.def.defName} from {_previousLevel} " + $"to {__instance.Level} ({__instance.LevelDescriptor})";
			Cache.Get(___pawn)?.AddTalkRequest(prompt, null, TalkType.LevelUp);
		}
	}
}
