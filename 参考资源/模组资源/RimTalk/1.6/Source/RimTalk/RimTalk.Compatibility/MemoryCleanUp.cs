...
public class MemoryCleanUp : GameComponent
{
	...
	{
		string[] cumulativeDefs = new string[2] { "Chitchat", "RimTalk_Chitchat" };
		int fixedCount = 0;
		...
		foreach (Map map in Current.Game.Maps)
		{
			...
			{
				MemoryThoughtHandler memHandler = pawn.needs?.mood?.thoughts?.memories;
				if (memHandler == null)
				{
					continue;
				}
				...
				{
					Thought_Memory memory = memHandler.Memories[i];
					if (cumulativeDefs.Contains(memory.def.defName) && !(memory is Thought_MemorySocialCumulative))
					{
