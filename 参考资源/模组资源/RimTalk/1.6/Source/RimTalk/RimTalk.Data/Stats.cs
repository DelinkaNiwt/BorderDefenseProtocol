...
public static class Stats
{
	...
	static Stats()
	{
		TokensPerMinuteHistory = new List<long>();
		CallsPerMinuteHistory = new List<long>();
		AvgTokensPerCallHistory = new List<long>();
		TokensPerSecondHistory = new List<long>();
		AvgTokensPerCallPerSecondHistory = new List<long>();
		Reset();
	}
	...
	public static void Update()
	{
		...
		{
			AvgCallsPerMinute = (double)TotalCalls / elapsedMinutes;
			AvgTokensPerMinute = (double)TotalTokens / elapsedMinutes;
			AvgTokensPerCall = ((TotalCalls > 0) ? ((double)TotalTokens / (double)TotalCalls) : 0.0);
		}
		if (DateTime.Now >= _nextSecondRolloverTime)
		{
			...
			while (TokensPerSecondHistory.Count > 60)
			{
				TokensPerSecondHistory.RemoveAt(0);
			}
			while (AvgTokensPerCallPerSecondHistory.Count > 60)
			{
				AvgTokensPerCallPerSecondHistory.RemoveAt(0);
			}
			_tokensThisSecond = 0L;
			_callsThisSecond = 0L;
			_nextSecondRolloverTime = _nextSecondRolloverTime.AddSeconds(1.0);
			while (_nextSecondRolloverTime < DateTime.Now)
			{
				TokensPerSecondHistory.Add(0L);
				AvgTokensPerCallPerSecondHistory.Add(0L);
				_nextSecondRolloverTime = _nextSecondRolloverTime.AddSeconds(1.0);
			}
		}
		if (!(DateTime.Now < _nextMinuteRolloverTime))
		{
			...
			while (TokensPerMinuteHistory.Count > 60)
			{
				TokensPerMinuteHistory.RemoveAt(0);
			}
			while (CallsPerMinuteHistory.Count > 60)
			{
				CallsPerMinuteHistory.RemoveAt(0);
			}
			while (AvgTokensPerCallHistory.Count > 60)
			{
				AvgTokensPerCallHistory.RemoveAt(0);
			}
			_tokensThisMinute = 0L;
			_callsThisMinute = 0L;
			_nextMinuteRolloverTime = _nextMinuteRolloverTime.AddMinutes(1.0);
			while (_nextMinuteRolloverTime < DateTime.Now)
			{
				TokensPerMinuteHistory.Add(0L);
				CallsPerMinuteHistory.Add(0L);
				AvgTokensPerCallHistory.Add(0L);
				_nextMinuteRolloverTime = _nextMinuteRolloverTime.AddMinutes(1.0);
			}
		}
	}
	...
	public static void Reset()
	{
		...
		_nextMinuteRolloverTime = DateTime.Now.AddMinutes(1.0);
		_nextSecondRolloverTime = DateTime.Now.AddSeconds(1.0);
		AvgCallsPerMinute = 0.0;
		AvgTokensPerMinute = 0.0;
