...
public static class AIErrorHandler
{
	...
	public static async Task<T> HandleWithRetry<T>(Func<Task<T>> operation, Action<Exception> onFailure = null)
	{
		...
		catch (Exception ex)
		{
			...
			string nextModel = settings.GetCurrentModel();
			...
			{
				Logger.Warning("Retry failed: " + ex2.Message);
				HandleFinalFailure(ex);
				onFailure?.Invoke(ex2);
				return default(T);
			}
		}
	}
	...
	private static bool CanRetryGeneration(RimTalkSettings settings)
	{
		if (settings.UseSimpleConfig)
		{
			if (settings.IsUsingFallbackModel)
			{
				return false;
			}
	...
	private static void ShowGenerationWarning(Exception ex)
	{
		...
		string message = string.Format("{0}: {1}", "RimTalk.TalkService.GenerationFailed".Translate(), ex.Message);
		Messages.Message(message, MessageTypeDefOf.NeutralEvent, historical: false);
	}
	...
	private static void ShowRetryMessage(Exception ex, string nextModel)
	{
		...
		string message = string.Format("{0}. {1}", messageKey.Translate(), "RimTalk.TalkService.TryingNextAPI".Translate(nextModel));
		Messages.Message(message, MessageTypeDefOf.NeutralEvent, historical: false);
	}
}
