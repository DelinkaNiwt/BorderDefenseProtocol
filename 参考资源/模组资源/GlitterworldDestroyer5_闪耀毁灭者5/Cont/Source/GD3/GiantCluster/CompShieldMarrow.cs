...
namespace GD3
{
	public class CompShieldMarrow : ThingComp
	{
		public CompProperties_ShieldMarrow Props
		{
			get
			{
				return (CompProperties_ShieldMarrow)this.props;
			}
		}
		...
		public override void PostDestroy(DestroyMode mode, Map previousMap)
		{
			...
			Messages.Message("MarrowADestroy".Translate(), MessageTypeDefOf.NeutralEvent);
			float proj = Find.ResearchManager.GetProgress(GDDefOf.GD3_GiantCluster_Medium);
			if (proj >= GDDefOf.GD3_GiantCluster_Medium.baseCost)
			{
				return;
			}
			Messages.Message("TechAFinished".Translate(), MessageTypeDefOf.NeutralEvent);
			Find.ResearchManager.FinishProject(GDDefOf.GD3_GiantCluster_Medium);
			...
			for (int j = 0; j < quests.Count; j++)
			{
				...
				{
					if (quest.root.defName == "GD_Quest_Cluster_M")
					{
			...
			if (flag)
			{
				...
				if (readyToUseTicks >= 60)
				{
					...
					for (int i = 0; i < Pawns.Count; i++)
					{
						...
						AddRadioHediff(pawn, GDDefOf.PsychicShieldMarrowHediff);
					}
				}
			}
		}
