...
namespace GD3
{
    public class MechClusterGenerator_Giant
    {
		...
		private static readonly SimpleCurve MortarShieldChanceCurve = new SimpleCurve
	{
		...
		public static void ResolveSketch(SketchResolveParams parms)
		{
			...
			List<ThingDef> buildingDefsForCluster = GetBuildingDefsForCluster(num, intVec, canBeDormant, value, false);
		...
		private static List<ThingDef> GetBuildingDefsForCluster(float points, IntVec2 size, bool canBeDormant, float? totalPoints, bool forceNoConditionCauser)
		{
			...
			List<ThingDef> source = DefDatabase<ThingDef>.AllDefsListForReading.Where((ThingDef def) => def.building?.buildingTags != null && def.building.buildingTags.Contains("MechClusterMember") && (!totalPoints.HasValue || (float)def.building.minMechClusterPoints <= totalPoints) || def.building?.buildingTags != null && def.building.buildingTags.Contains("GiantTurretRevenge")).ToList();
			if (!forceNoConditionCauser)
			{
				int num = GenMath.RoundRandom(ProblemCauserCountCurve.Evaluate(points));
				for (int i = 0; i < num; i++)
				{
					if (!source.Where((ThingDef x) => x.building.buildingTags.Contains("MechClusterProblemCauser_Mortar")).TryRandomElementByWeight((ThingDef t) => t.generateCommonality, out var result))
					{
						break;
					}
					list.Add(result);
