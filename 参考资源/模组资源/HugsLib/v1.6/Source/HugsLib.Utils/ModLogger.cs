...
public class ModLogger : IModLogger
{
	...
	{
		Log.Warning(FormatOutput(message, "warn", substitutions));
	}
	...
	{
		Log.Error(FormatOutput(message, "ERR", substitutions));
	}
	...
	public void Trace(params object[] strings)
	{
		if (Prefs.DevMode)
		{
			...
			{
				strings = new object[1] { GetModPrefix() };
			}
	...
	public void ReportException(Exception e, string modIdentifier = null, bool reportOnceOnly = false, string location = null)
	{
		if (location == null)
		{
			location = new StackFrame(1, fNeedFileInfo: true).GetMethod().Name;
		}
		if (!reportOnceOnly || !(lastExceptionLocation == location))
		{
			lastExceptionLocation = location;
			string text = ((modIdentifier == null) ? FormatOutput("Exception during {0}: {1}", "ERR", location, e) : FormatOutput("{0} caused an exception during {1}: {2}", "ERR", modIdentifier, location, e));
			Log.Error(text);
		}
	}
