...
public class DoLaterScheduler
{
	...
	internal void OnUpdate()
	{
		...
		{
			InvokeCallbacks(nextUpdate);
		}
	}

	internal void OnGUI()
	{
		if (nextOnGUI.Count > 0)
		{
			InvokeCallbacks(nextOnGUI);
		}
	}

	internal void OnMapLoaded(Map map)
	{
		int count = nextMapLoaded.Count;
		while (count-- > 0)
		{
			Action<Map> action = nextMapLoaded.Dequeue();
			try
			{
				action(map);
			}
			...
			{
				HugsLibUtility.BlameCallbackException("DoLaterScheduler", action, e);
			}
		}
	}
