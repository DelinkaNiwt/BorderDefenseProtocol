...
internal class Dialog_LibraryUpdateRequired : Window
{
	...
	public Dialog_LibraryUpdateRequired(string requiringModName, Version requiredVersion)
	{
		...
		bodyText = "HugsLib_updateRequired_text".Translate().Formatted(requiringModName, requiredVersion.ToSemanticString());
		updateButtonText = "HugsLib_updateRequired_updateBtn".Translate();
		buttonSize = new Vector2(Mathf.Max(Window.CloseButSize.x, Text.CalcSize(updateButtonText).x + 20f), Window.CloseButSize.y);
		closeOnCancel = true;
		doCloseButton = false;
		doCloseX = false;
		forcePause = true;
	...
	public override void DoWindowContents(Rect inRect)
	{
		...
		{
			Rect rect = new Rect(inRect.width - buttonSize.x, inRect.height - buttonSize.y, buttonSize.x, buttonSize.y);
		...
		void DrawMainTextLabel()
		{
			...
			Rect rect = new Rect(inRect.x, inRect.y + 45f, inRect.width, inRect.height - buttonSize.y - 45f);
		...
		{
			Rect rect = new Rect(inRect.x, inRect.height - buttonSize.y, buttonSize.x, buttonSize.y);
	...
	{
		string url = (SteamManager.Initialized ? "steam://url/CommunityFilePage/818773962" : "https://github.com/UnlimitedHugs/RimworldHugsLib/releases/latest");
