...
public static class Shell
{
	...
	public class UnsupportedPlatformException : Exception
	{
		...
		{
			return $"{commandName} is not compatible with {UnityData.platform}";
		}

		public UnsupportedPlatformException(string commandName)
			: base(ExpandCommandName(commandName))
		{
		}

		public UnsupportedPlatformException(string commandName, Exception inner)
			: base(ExpandCommandName(commandName), inner)
		{
		}
	...
	public static bool StartProcess(ProcessStartInfo psi, ref Process process)
	{
		...
		{
			if (psi.FileName.StartsWith("\"") && psi.FileName.EndsWith("\""))
			{
				HugsLibController.Logger.Warning("Detected surround quotes on shell filename, some platforms interpret them as part of the filename.");
			}
