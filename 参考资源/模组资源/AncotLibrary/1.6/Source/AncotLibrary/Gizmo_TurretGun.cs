...
public class Gizmo_TurretGun : Gizmo_ActionAndToggle
{
	...
	private readonly List<CompTurretGun_Custom> comps = new List<CompTurretGun_Custom>();
	...
	public void AddComp(CompTurretGun_Custom comp)
	{
	...
	public override GizmoResult GizmoOnGUI(Vector2 topLeft, float maxWidth, GizmoRenderParms parms)
	{
		...
		Texture2D texture2D = (comps.Any((CompTurretGun_Custom c) => c.fireAtWill) ? Widgets.CheckboxOnTex : Widgets.CheckboxOffTex);
		GUI.DrawTexture(rect5, (Texture)texture2D);
		if (Widgets.ButtonInvisible(rect5))
		{
			foreach (CompTurretGun_Custom comp in comps)
			{
				comp.fireAtWill = !comp.fireAtWill;
				comp.PawnOwner?.Drawer?.renderer?.renderTree?.SetDirty();
		...
		GUI.DrawTexture(rect6, (Texture)AncotLibraryIcon.VanillaHalt);
		...
		{
			foreach (CompTurretGun_Custom comp2 in comps)
			{
	...
	private void Update(LocalTargetInfo target)
	{
		...
		foreach (CompTurretGun_Custom comp in comps)
		{
			if (comp.PawnOwner.Position.IsValid)
			{
				if (range > 0f)
				{
					GenDraw.DrawRadiusRing(comp.PawnOwner.Position, range);
				}
				if (minRange > 0f)
				{
					GenDraw.DrawRadiusRing(comp.PawnOwner.Position, minRange);
				}
			}
		}
	}
	...
	{
		if (other is Gizmo_TurretGun gizmo_TurretGun)
		{
			return gizmo_TurretGun.defaultLabel == defaultLabel && gizmo_TurretGun.icon == icon;
		}
		return false;
	}
	...
	public override void MergeWith(Gizmo other)
	{
		...
		foreach (CompTurretGun_Custom comp in gizmo_TurretGun.comps)
		{
			AddComp(comp);
		}
	}
