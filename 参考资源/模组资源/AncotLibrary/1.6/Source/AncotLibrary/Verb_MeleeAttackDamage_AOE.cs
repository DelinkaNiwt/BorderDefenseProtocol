...
{
	public MeleeAttackAOE_Extension Extension => maneuver?.GetModExtension<MeleeAttackAOE_Extension>();
	...
	public virtual List<Pawn> TargetPawns()
	{
		...
		Vector3 v = (thing.Position - CasterPawn.Position).ToVector3();
		v.Normalize();
		float directionAngle = v.ToAngleFlat();
		Pawn pawn = thing as Pawn;
		if (Extension.requireCharge)
		{
		...
		compCharge?.UsedOnce();
		EffecterTrigger(thing);
		foreach (Thing item in GenRadial.RadialDistinctThingsAround(CasterPawn.Position, CasterPawn.Map, Extension.radius, useCenter: true))
		{
			if (item is Pawn pawn2 && pawn2 != CasterPawn && pawn2 != pawn && AreaFinder.ConeShapedArea(CasterPawn.Position, pawn2.Position, Extension.angle, directionAngle, Extension.radius) && (pawn2.Faction == null || CasterPawn.Faction == null || (pawn2.Faction != CasterPawn.Faction && pawn2.Faction.RelationKindWith(CasterPawn.Faction) == FactionRelationKind.Hostile)))
			{
				list.Add(pawn2);
	...
	protected override bool TryCastShot()
	{
		...
		List<Pawn> list = TargetPawns();
		List<Thing> list2 = new List<Thing>();
		if (list.NullOrEmpty())
		{
			return base.TryCastShot();
		}
		if (list.Count > Extension.maxHitTarget - 1)
		{
			for (int i = 0; i < Extension.maxHitTarget - 1; i++)
			{
				Pawn item = list.RandomElement();
				list2.Add(item);
				list.Remove(item);
			}
		}
		...
		foreach (Thing item2 in list2)
		{
			...
			Map map = item2.Map;
			Vector3 drawPos = item2.DrawPos;
			SoundDef soundDef = null;
			bool flag = item2 == thing;
			if (Rand.Chance(GetNonMissChance(item2)))
			{
				if (!Rand.Chance(GetDodgeChance(item2)))
				{
					soundDef = ((item2.def.category != ThingCategory.Building) ? SoundHitPawn() : SoundHitBuilding());
					...
					BattleLogEntry_MeleeCombat battleLogEntry_MeleeCombat = CreateCombatLog((ManeuverDef maneuver) => maneuver.combatLogRulesHit, alwaysShow: true, item2);
					if (flag)
					{
						result = true;
					}
					DamageWorker.DamageResult damageResult = ApplyMeleeDamageToTarget(item2);
			...
			if (soundDef != null && flag)
			{
				soundDef.PlayOneShot(new TargetInfo(item2.Position, map));
			}
			if (casterPawn.Spawned && flag)
			{
				casterPawn.Drawer.Notify_MeleeAttackOn(item2);
			}
			...
			if (casterPawn.caller != null && flag)
			{
	...
	{
		return CasterPawn != null && CasterPawn.IsColonist && !CasterPawn.IsSlave && targetPawn != null && targetPawn.IsSlaveOfColony && targetPawn.health.capacities.CanBeAwake && !SlaveRebellionUtility.IsRebelling(targetPawn);
	}
	...
	private bool IsTargetImmobile(LocalTargetInfo target)
	{
		...
		Pawn pawn = thing as Pawn;
		return thing.def.category != ThingCategory.Pawn || pawn.Downed || (int)pawn.GetPosture() > 0;
	}
	...
	public BattleLogEntry_MeleeCombat CreateCombatLog(Func<ManeuverDef, RulePackDef> rulePackGetter, bool alwaysShow, Thing target)
	{
		...
		BattleLogEntry_MeleeCombat battleLogEntry_MeleeCombat = new BattleLogEntry_MeleeCombat(rulePackGetter(maneuver), alwaysShow, CasterPawn, target, base.ImplementOwnerType, tool.labelUsedInLogging ? tool.label : "", (base.EquipmentSource == null) ? null : base.EquipmentSource.def, (base.HediffCompSource == null) ? null : base.HediffCompSource.Def, maneuver.logEntryDef);
