...
public class RaidStrategyWorker_ImmediateAttack_Psycasters : RaidStrategyWorker_ImmediateAttack
{
	...
	{
		return ((Def)kind).HasModExtension<PawnKindAbilityExtension_Psycasts>();
	}
	...
	{
		if (chosenGroups != null && chosenGroups.Count < MinRequiredPawnsForPoints(pointsTotal, faction) && !MatchesRequiredPawnKind(g.kind))
		{
			return false;
		}
		return true;
