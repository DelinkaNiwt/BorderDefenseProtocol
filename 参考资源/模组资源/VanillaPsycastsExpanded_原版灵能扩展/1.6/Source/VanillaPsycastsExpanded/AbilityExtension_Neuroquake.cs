...
public class AbilityExtension_Neuroquake : AbilityExtension_AbilityMod
{
	...
	public override void Cast(GlobalTargetInfo[] targets, Ability ability)
	{
		...
		foreach (Pawn item in ability.pawn.Map.mapPawns.AllPawnsSpawned)
		{
			...
			{
				bool flag = !item.Spawned || item.Position.InHorDistOf(ability.pawn.Position, ability.GetAdditionalRadius()) || !item.Position.InHorDistOf(ability.pawn.Position, ability.GetRadiusForPawn());
		...
		{
			AbilityExtension_GiveMentalState.TryGiveMentalStateWithDuration(item2.RaceProps.IsMechanoid ? MentalStateDefOf.BerserkMechanoid : MentalStateDefOf.Berserk, item2, ability, StatDefOf.PsychicSensitivity, durationScalesWithCaster: false);
