...
internal static class PlatformDetection
{
	...
	private unsafe static void DetectInfoPosix(ref OSKind os, ref ArchitectureKind arch)
	{
		...
		try
		{
			...
			if (self.Contains("X86_64", StringComparison.Ordinal))
			{
				arch = ArchitectureKind.x86_64;
			}
			else if (self.Contains("AMD64", StringComparison.Ordinal))
			{
				arch = ArchitectureKind.x86_64;
			}
			else if (self.Contains("X86", StringComparison.Ordinal))
			{
				arch = ArchitectureKind.x86;
			}
			else if (self.Contains("AARCH64", StringComparison.Ordinal))
			{
				arch = ArchitectureKind.Arm64;
			}
			else if (self.Contains("ARM64", StringComparison.Ordinal))
			{
				arch = ArchitectureKind.Arm64;
			}
	...
	{
		string text = null;
		MMDbgLog.DebugLogTraceStringHandler message;
		bool isEnabled;
		int i;
		if (os == OSKind.Linux)
		{
			if (DynDll.OpenLibrary("libc").TryGetExport("getauxval", out var functionPtr))
			{
				...
				if (intPtr != (IntPtr)0)
				{
					...
					message = new MMDbgLog.DebugLogTraceStringHandler(35, 1, out isEnabled);
					if (isEnabled)
					{
			...
			if (text == null)
			{
				MMDbgLog.DebugLogWarningStringHandler message2;
				try
				{
					...
					if (text.Length == 0)
					{
						message2 = new MMDbgLog.DebugLogWarningStringHandler(56, 1, out isEnabled);
						if (isEnabled)
						{
						...
						for (i = 0; i < span2.Length; i++)
						{
							Unix.LinuxAuxvEntry linuxAuxvEntry2 = span2[i];
							message = new MMDbgLog.DebugLogTraceStringHandler(3, 2, out isEnabled);
							if (isEnabled)
							{
								message.AppendFormatted(linuxAuxvEntry2.Key, "x16");
								message.AppendLiteral(" = ");
								message.AppendFormatted(linuxAuxvEntry2.Value, "x16");
							}
							MMDbgLog.Trace(ref message);
						}
						text = null;
					...
					{
						message = new MMDbgLog.DebugLogTraceStringHandler(43, 1, out isEnabled);
						if (isEnabled)
						{
							message.AppendLiteral("Got architecture name ");
							message.AppendFormatted(text);
							message.AppendLiteral(" from /proc/self/auxv");
						}
						MMDbgLog.Trace(ref message);
					}
				}
				catch (UnauthorizedAccessException value)
				{
					...
					message2 = new MMDbgLog.DebugLogWarningStringHandler(0, 1, out isEnabled);
					if (isEnabled)
					{
						message2.AppendFormatted(value);
					}
		...
		if (text == null)
		{
			...
			if (isEnabled)
			{
				message.AppendLiteral("Got architecture name ");
				message.AppendFormatted(text);
				message.AppendLiteral(" from uname()");
			}
			MMDbgLog.Trace(ref message);
		}
		return text;
	...
	private static (RuntimeKind Rt, CorelibKind Cor, Version Ver) DetermineRuntimeInfo()
	{
		...
		MMDbgLog.DebugLogInfoStringHandler message3 = new MMDbgLog.DebugLogInfoStringHandler(34, 3, out isEnabled);
		if (isEnabled)
		{
			...
			message3.AppendLiteral(" using ");
			message3.AppendFormatted(corelibKind);
			message3.AppendLiteral(" corelib");
		}
