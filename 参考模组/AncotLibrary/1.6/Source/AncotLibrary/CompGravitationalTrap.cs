...
public class CompGravitationalTrap : ThingComp
{
	...
	public CompProperties_GravitationalTrap Props => (CompProperties_GravitationalTrap)props;
	...
	protected void Detonate(Map map, bool ignoreUnspawned = false)
	{
		foreach (Pawn item in parent.Map.mapPawns.AllPawnsSpawned)
		{
			...
			{
				ForceMovementUtility.ApplyGravitationalForce(parent.PositionHeld, item, Props.distance, Props.removeHediffsAffected, ignoreResistance: false, Props.fieldStrength);
			}
		}
		...
		else if (Props.explosionEffect != null)
		{
