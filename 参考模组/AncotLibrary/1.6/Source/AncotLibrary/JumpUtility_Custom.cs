...
public static class JumpUtility_Custom
{
	public static bool DoJump(Pawn pawn, LocalTargetInfo currentTarget, CompApparelReloadable comp, VerbProperties verbProps, Ability triggeringAbility = null, LocalTargetInfo target = default(LocalTargetInfo), ThingDef pawnFlyerOverride = null)
	{
		...
		PawnJumper_Custom pawnJumper_Custom = PawnJumper_Custom.MakeFlyer(pawnFlyerOverride ?? AncotDefOf.Ancot_PawnFlyer, pawn, cell, verbProps.flightEffecterDef, verbProps.soundLanding, verbProps.flyWithCarriedThing, null, triggeringAbility, target);
		if (pawnJumper_Custom != null)
		{
			FleckMaker.ThrowDustPuff(position.ToVector3Shifted() + Gen.RandomHorizontalVector(0.5f), map, 2f);
			GenSpawn.Spawn(pawnJumper_Custom, cell, map);
			if (flag)
			{
