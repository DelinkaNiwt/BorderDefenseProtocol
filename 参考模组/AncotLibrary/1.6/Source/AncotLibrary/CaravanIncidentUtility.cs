...
public static class CaravanIncidentUtility
{
	...
	public static Map SetupCaravanAttackMap(Caravan caravan, List<Pawn> enemies, bool sendLetterIfRelatedPawns, out IntVec3 playerSpot, out IntVec3 secondSpot)
	{
		int num = CalculateIncidentMapSize(caravan.PawnsListForReading, enemies);
		Map map = RimWorld.Planet.CaravanIncidentUtility.GetOrGenerateMapForIncident(caravan, new IntVec3(num, 1, num), WorldObjectDefOf.Ambush);
		MultipleCaravansCellFinder.FindStartingCellsFor2Groups(map, out var playerStartingSpot, out var second);
		playerSpot = playerStartingSpot;
		secondSpot = second;
