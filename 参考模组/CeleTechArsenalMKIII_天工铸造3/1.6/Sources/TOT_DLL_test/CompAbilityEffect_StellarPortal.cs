...
public class CompAbilityEffect_StellarPortal : CompAbilityEffect
{
	...
	public new CompProperties_AbilityStellarPortal Props => (CompProperties_AbilityStellarPortal)props;

	public override void Apply(LocalTargetInfo target, LocalTargetInfo dest)
	{
		base.Apply(target, dest);
		Map map = parent.pawn.Map;
		named = DefDatabase<ThingDef>.GetNamed("CMC_StellarPortal");
		...
		foreach (Thing item3 in list)
		{
			...
			{
				GenPlace.TryPlaceThing(item3, item3.Position, map, ThingPlaceMode.Near, null, null, default(Rot4));
			}
		}
	}
