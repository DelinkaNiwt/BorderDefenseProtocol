...
public class SketchResolver_MiliraClusterWalls : SketchResolver
{
	...
	private void TryAddWall(Sketch sketch, IntVec2 size)
	{
		for (int i = 0; i < 50; i++)
		{
			...
			CellRect rect2 = ((!Rand.Bool) ? new CellRect(intVec3.x - num2 + 1, intVec3.z, num2, 1) : new CellRect(intVec3.x, intVec3.z, num2, 1));
			rect2.ClipInsideRect(new CellRect(0, 0, size.x, size.z));
			if (rect2.Area >= 2)
			{
				int num3 = GenMath.RoundRandom(Rand.Range((float)size.z * 0.4f, size.z));
				CellRect rect3 = ((!Rand.Bool) ? new CellRect(intVec3.x, intVec3.z - num3 + 1, 1, num3) : new CellRect(intVec3.x, intVec3.z, 1, num3));
		...
		void GenerateWallInRect(CellRect cellRect, bool createRandomGap)
		{
			...
			foreach (IntVec3 item in cellRect)
			{
				...
				{
					sketch.AddThing(ThingDefOf.Wall, item, Rot4.North, MiliraDefOf.Milira_SunPlateSteel);
				}
			}
		}
