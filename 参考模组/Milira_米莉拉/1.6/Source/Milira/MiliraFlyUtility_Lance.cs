...
public static class MiliraFlyUtility_Lance
{
	...
	public static bool DoJump(Pawn pawn, LocalTargetInfo currentTarget, CompWeaponCharge comp, VerbProperties verbProps, Ability triggeringAbility = null, LocalTargetInfo target = default(LocalTargetInfo), ThingDef pawnFlyerOverride = null)
	{
		...
		MiliraPawnFlyer_Lance miliraPawnFlyer_Lance = MiliraPawnFlyer_Lance.MakeFlyer(pawnFlyerOverride ?? MiliraDefOf.Milira_PawnJumper_Lance, pawn, intVec, verbProps.flightEffecterDef, verbProps.soundLanding, verbProps.flyWithCarriedThing, null, triggeringAbility, target);
		if (miliraPawnFlyer_Lance != null)
		{
			FleckMaker.ThrowDustPuff(position.ToVector3Shifted() - vector, map, 2f);
			GenSpawn.Spawn(miliraPawnFlyer_Lance, intVec, map);
			if (flag)
			{
