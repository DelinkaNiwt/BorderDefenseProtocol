...
public static class MiliraClusterUtility
{
	...
	public static IntVec3 FindClusterPosition(Map map, MechClusterSketch sketch, int maxTries = 100, float spawnCloseToColonyChance = 0f)
	{
		...
		{
			IntVec3 entrySpot = CellFinderLoose.RandomCellWith((IntVec3 x) => x.Standable(map), map);
			if (!entrySpot.IsValid)
			{
				continue;
			}
			IntVec3 intVec = RCellFinder.FindSiegePositionFrom(entrySpot, map, allowRoofed: false, errorOnFail: false);
			if (intVec.IsValid)
			{
				float clusterPositionScore2 = GetClusterPositionScore(intVec, map, sketch);
	...
	public static List<Thing> SpawnCluster(IntVec3 center, Map map, MechClusterSketch sketch, bool dropInPods = true, bool canAssaultColony = false, string questTag = "MilianCluster")
	{
		...
		sketch.buildingsSketch.Spawn(map, center, miliraFaction, Sketch.SpawnPosType.Unchanged, spawnMode, wipeIfCollides: false, forceTerrainAffordance: true, clearEdificeWhereFloor: false, spawnedThings, sketch.startDormant, buildRoofsInstantly: false, CanSpawnThing, delegate(IntVec3 spot, SketchEntity entity)
		{
			if (entity is SketchThing sketchThing && sketchThing.def != ThingDefOf.Wall && sketchThing.def != ThingDefOf.Barricade)
			{
				entity.SpawnNear(spot, map, 12f, miliraFaction, spawnMode, wipeIfCollides: false, forceTerrainAffordance: false, spawnedThings, sketch.startDormant, CanSpawnThing);
			}
		});
		...
		if (!sketch.pawns.NullOrEmpty())
		{
			foreach (MechClusterSketch.Mech pawn2 in sketch.pawns)
			{
				...
				if (dropInPods)
				{
					...
					DropPodUtility.MakeDropPodAt(result, map, activeTransporterInfo, miliraFaction);
				}
				else
				{
