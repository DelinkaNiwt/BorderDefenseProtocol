...
public class ThingCarrierGizmo : Gizmo
{
	public CompThingCarrier_Custom carrier;
	...
	public ThingCarrierGizmo(CompThingCarrier_Custom carrier)
	{
		this.carrier = carrier;
		targetValue = (float)carrier.maxToFill / (float)carrier.MaxIngredientCount;
	...
	public override GizmoResult GizmoOnGUI(Vector2 topLeft, float maxWidth, GizmoRenderParms parms)
	{
		...
		rect4.y -= 2f;
		Widgets.Label(rect4, carrier.IngredientCount + " / " + carrier.MaxIngredientCount);
		Text.Anchor = TextAnchor.UpperLeft;
		TooltipHandler.TipRegion(rect4, () => GetResourceBarTip(), Gen.HashCombineInt(carrier.GetHashCode(), 34244569));
		if (lastTargetValue != targetValue)
		{
			carrier.maxToFill = Mathf.RoundToInt(targetValue * (float)carrier.MaxIngredientCount);
		}
	...
	private string GetResourceBarTip()
	{
		...
		stringBuilder.AppendInNewLine(carrier.GizmoDesc);
