...
public static class MiliraFlyUtility_Blade
{
	public static bool DoJump(Pawn pawn, LocalTargetInfo currentTarget, CompWeaponCharge comp, VerbProperties verbProps, Ability triggeringAbility = null, LocalTargetInfo target = default(LocalTargetInfo), ThingDef pawnFlyerOverride = null)
	{
		...
		MiliraPawnFlyer_Blade miliraPawnFlyer_Blade = MiliraPawnFlyer_Blade.MakeFlyer(pawnFlyerOverride ?? MiliraDefOf.Milira_PawnJumper_Blade, pawn, cell, verbProps.flightEffecterDef, verbProps.soundLanding, verbProps.flyWithCarriedThing, null, triggeringAbility, target);
		if (miliraPawnFlyer_Blade != null)
		{
			...
			GenSpawn.Spawn(miliraPawnFlyer_Blade, cell, map);
			if (flag)
			{
