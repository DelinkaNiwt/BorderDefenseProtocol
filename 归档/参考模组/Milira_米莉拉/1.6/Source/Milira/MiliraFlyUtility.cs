...
public static class MiliraFlyUtility
{
	public static bool DoJump(Pawn pawn, LocalTargetInfo currentTarget, CompApparelReloadable comp, VerbProperties verbProps, Ability triggeringAbility = null, LocalTargetInfo target = default(LocalTargetInfo), ThingDef pawnFlyerOverride = null)
	{
		...
		MiliraPawnFlyer miliraPawnFlyer = MiliraPawnFlyer.MakeFlyer(pawnFlyerOverride ?? MiliraDefOf.Milira_PawnJumper, pawn, cell, verbProps.flightEffecterDef, verbProps.soundLanding, verbProps.flyWithCarriedThing, null, triggeringAbility, target);
		if (miliraPawnFlyer != null)
		{
			FleckMaker.ThrowDustPuff(position.ToVector3Shifted() - vector, map, 2f);
			GenSpawn.Spawn(miliraPawnFlyer, cell, map);
			if (flag)
			{
