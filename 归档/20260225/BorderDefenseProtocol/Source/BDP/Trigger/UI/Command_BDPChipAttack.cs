...
namespace BDP.Trigger
{
    ...
    public class Command_BDPChipAttack : Command_VerbTarget
    {
        ...
        ///
        /// 左键拦截（v7.0新增）：
        ///   Interacted + Verb_BDPGuided → StartGuidedTargeting，返回Clear阻止ProcessInput。
        ///   原因：ProcessInput调用BeginTargeting(verb)使用verb.targetParams（不允许地面瞄准），
        ///     且选中目标后OrderForceTarget会丢弃第一个目标。
        ///
        /// 右键拦截（v6.1）：
        ///   OpenedFloatMenu + volleyVerb → 齐射瞄准，返回Clear阻止浮动菜单。
        ///   引导齐射verb直接调用StartGuidedTargeting，普通齐射走原版BeginTargeting。
        /// </summary>
        protected override GizmoResult GizmoOnGUIInt(Rect butRect, GizmoRenderParms parms)
        {
            var result = base.GizmoOnGUIInt(butRect, parms);

            // 拦截左键：引导弹verb直接启动多步锚点瞄准，绕过原版targeting流程
            if (result.State == GizmoState.Interacted && verb is Verb_BDPGuided guided)
            {
                SoundDefOf.Tick_Tiny.PlayOneShotOnCamera();
                guided.StartGuidedTargeting();
                return new GizmoResult(GizmoState.Clear);
            }
            // v7.0：双手触发verb含变化弹侧时，左键也启动锚点瞄准
            if (result.State == GizmoState.Interacted && verb is Verb_BDPDualRanged dual && dual.HasGuidedSide)
            {
                SoundDefOf.Tick_Tiny.PlayOneShotOnCamera();
            ...
            if (result.State == GizmoState.OpenedFloatMenu && volleyVerb != null)
            {
                SoundDefOf.Tick_Tiny.PlayOneShotOnCamera();
                // 引导齐射verb直接启动多步锚点瞄准
                if (volleyVerb is Verb_BDPGuidedVolley guidedVolley)
                {
                    guidedVolley.StartGuidedTargeting();
                }
                // v7.0：双侧齐射verb含变化弹侧时，也启动锚点瞄准
